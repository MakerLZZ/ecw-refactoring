...
export default class GoodsDetailRight extends Component {
	MapItem = (flag) => {
		if (flag) {
			return msg_1.map((v) => {
			...
			<div className="goods-detail-right">
				...
				<div className="goods-select-num">
					<div>
						...
						<span className="goods-select-stock">
							<span className={this.state.reduceClassName} onClick={() => this.handleReduceClick()}>
								-
							</span>
							<input
								ref="numInput"
								...
								maxLength="8"
							/>
							<span className={this.state.increaseClassName} onClick={() => this.handleIncreaseClick()}>
								+
							</span>
							件
						</span>
				...
				<div className="buy-box">
					<div className="buy">
						{this.props.loginSuccess ? (
							<Link to="/order_submit" className="btn-a">
								立即购买
							</Link>
						) : (
							<div className="btn-a" onClick={() => this.notLoginTip()}>
								立即购买
							</div>
						)}
					</div>
					<div className="add">
						{this.props.loginSuccess ? (
							<Link to="/personal_center/cart" className="btn-a">
								<Icon type="shopping-cart" className="add-icon" />加入购物车
							</Link>
						) : (
							<div className="btn-a" onClick={() => this.notLoginTip()}>
								加入购物车
				...
				<div className="bottom">
					<div className="bottom-row">
						...
						<div className="bottom-a">{this.MapItem(true)}</div>
					</div>
					<div className="bottom-row">
						<div className="bottom-title">
							<span>支付</span>
						</div>
						<div className="bottom-a">{this.MapItem(false)}</div>
					</div>
				</div>
			</div>
		);
